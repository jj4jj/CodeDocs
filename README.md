...
相较于原始 CodeWiki 项目，CodeDoc 重点增强了文档生成的速度、灵活性和可运营性：
...
- **🐹 Go + Protobuf 语言支持**：支持 `.go` 与 `.proto` 解析，包含 Protobuf message/service/rpc 的关系提取与文档化。
- **🧩 Monorepo 子项目与文档视图**：同一仓库可定义多个子项目（`subproject_path`），并按不同 `doc_type`（如 `architecture` / `developer`）生成与切换不同视图。
- **🗂️ 文档类型模板管理**：后端支持文档类型模板（Prompt、include/exclude/focus/skills/token 参数）持久化管理，并可在控制台维护。
- **🚦Web 任务并行执行（2~8）**：任务队列支持可配置并行度，默认 `2`，最大 `8`（`--task-concurrency` 或 `CODEWIKI_TASK_CONCURRENCY`）。
- **🕒 版本化输出与切换**：相同仓库/子项目重复生成时写入 `output/docs/<job_id>/<yymmdd-hhmmss>/`，文档页可直接切换版本。
- **💬 文档侧 Chat Agent（A2UI）**：文档页内置 `CodeWikiAgent`，会话本地持久化；默认只读工具链（文档/代码检索 + 受限 bash），并支持模型 fallback。
- **🔎 Mermaid 质量与阅读体验增强**：默认引入 `mermaid-validator` skill；文档页支持 Mermaid 双击独立视图、滚轮缩放、拖拽平移与 ESC 关闭。
...
CodeDoc 内置 FastAPI Web 服务，提供任务提交、并行队列执行、版本化文档浏览、模板管理与文档内 Chat Agent。

#### 启动 Web 服务

```bash
# 方式一：直接运行
python codewiki/run_web_app.py --host 127.0.0.1 --port 8000 --task-concurrency 2

# 方式二：模块方式
python -m codewiki.run_web_app --host 0.0.0.0 --port 8000 --task-concurrency 4
```
...
| 特性 | 说明 |
|------|------|
| **并行任务队列** | 可配置并行执行数（默认 `2`，最大 `8`），支持命令行 `--task-concurrency` 与环境变量 `CODEWIKI_TASK_CONCURRENCY` |
| **Monorepo 子项目** | 可为同一仓库定义多个子项目（`subproject_name` + `subproject_path`），并在文档页切换 |
| **文档视图（doc_type）** | 同一仓库/子项目可生成多个文档视图（如 `developer` / `architecture`），并在文档页切换 |
| **文档类型模板管理** | 支持内置 + 自定义文档类型模板（Prompt、patterns、focus、skills、token 参数） |
| **版本化文档输出** | 输出目录默认带时间版本：`output/docs/<job_id>/<yymmdd-hhmmss>/`，文档页可切换版本 |
| **任务参数回填重新生成** | 控制台“重新生成”会回填原参数到“新建任务”面板，支持手动确认后提交 |
| **进度与日志** | 任务状态与模块级进度实时展示；支持 `-v/--verbose` 扩展日志 |
| **文档内 Chat Agent** | 文档页右侧 `CodeWikiAgent`（A2UI），会话本地持久化，只读访问文档与代码目录 |
| **Chat 过程可观测** | 支持等待阶段动态状态、回答打字机效果、思考/工具/SKILL/内容分块展示（可折叠） |
| **Mermaid 阅读增强** | Mermaid 图支持双击独立视图、滚轮缩放、拖拽平移、双击复位、ESC 关闭 |
...
| `/api/tasks` | POST | 提交新任务，支持 `subproject_* / doc_type / skills / custom_cli_args / concurrency` 等参数 |
| `/api/tasks/{job_id}` | DELETE | 删除指定任务 |
| `/api/tasks/{job_id}/regenerate` | POST | 复用既有参数生成新版本任务 |
| `/api/tasks/{job_id}/stop` | POST | 停止运行中的任务 |
| `/api/job/{job_id}` | GET | 获取单个任务状态 |
| `/api/doc-types` | GET | 获取文档类型模板列表（内置+自定义） |
| `/api/doc-types/{doc_type}` | PUT | 新增/更新文档类型模板 |
| `/api/doc-types/{doc_type}` | DELETE | 删除自定义模板（内置模板恢复默认） |
| `/api/docs/{job_id}/chat` | POST | 文档页 Chat Agent 问答（A2UI 协议） |
...
# 创建/更新文档类型模板
curl -X PUT "http://localhost:8000/api/doc-types/developer-plus" \
  -H "Content-Type: application/json" \
  -d '{"display_name":"Developer Plus","prompt":"Focus on deep implementation details.","skills":["mermaid-validator"],"max_depth":4}'

# 文档页 Chat（A2UI）
curl -X POST "http://localhost:8000/api/docs/o1--server/chat" \
  -H "Content-Type: application/json" \
  -d '{"protocol":"a2ui-0.1","message":"请解释订单模块核心调用链","current_page":"overview.md"}'
```

#### Web/Chat 常用环境变量

```bash
# Web Worker 并行任务数（默认 2，最大 8）
export CODEWIKI_TASK_CONCURRENCY=4

# 文档 Chat Agent 模型配置（支持 fallback 顺序）
export AGENT_MODEL_API_KEY=your_key
export AGENT_MODEL_BASE_URL=https://api.anthropic.com/v1
export AGENT_MODEL_NAMES='["kimi-k2.5","claude-3-7-sonnet"]'
```
