...
4. 若配置了子项目目录（`subproject_path`），则在克隆仓库中切到对应子目录作为分析根目录。
5. 调用 `DocumentationGenerator.run()`。
6. 写缓存索引、任务状态、日志文件。
7. 提供 `/docs/{job_id}` 与 `/static-docs/...` 浏览。
...
5. `--index-page`
6. `--no-cache`
7. `--include/--exclude/--focus/--doc-type/--instructions/--skills`
...
5. 若参数影响任务维度（如 `subproject_path`），同步调整 `job_id` 与缓存 scope 生成规则。
